...
function setConnState(s) {
  ...
  connDot.title={checking:'Connectingâ€¦',online:'Server connected',offline:'Server offline'}[s];
}

/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Offline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ User bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Class picker â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function selectClass(classId) {
  ...
  document.getElementById('class-pill-name').textContent=cls?.name||'â€”';
...
async function loadRegisterClasses() {
  ...
  try {
    ...
    sel.innerHTML=classes.length
      ?'<option value="">Choose your classâ€¦</option>'+classes.map(c=>`<option value="${esc(c.id)}">${esc(c.name)}</option>`).join('')
...
async function doRegister() {
  ...
  try {
    ...
    err.textContent='Cannot reach server â€” is it running?'; err.style.display='block';
  }
}
...
async function doLogin() {
  ...
  try {
    ...
    err.textContent='Cannot reach server â€” is it running?';
    err.style.display='block';
  }
}
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Users modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function loadUserList() {
  ...
  list.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:6px 4px">Loadingâ€¦</p>';
  try {
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Topics panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function loadTopicsPanel() {
  ...
  nameInput.type='text'; nameInput.placeholder='New topic nameâ€¦'; nameInput.autocomplete='off'; nameInput.spellcheck=false;
...
function renderTopicCards(container) {
  ...
  topics.forEach(topic=>{
    ...
    const dropLabel=mk('span','tp-drop-label'); dropLabel.textContent='Click to upload files â€” or drag them here';
...
async function loadDocumentTags(filesWrap, topicId) {
  ...
  try {
    ...
    docs.forEach(doc=>{
      ...
      tag.innerHTML=`${esc(doc.filename)}<button class="tp-src-del" title="Delete">Ã—</button>`;
...
function handleFiles(files, topicId, uploadingWrap, filesWrap) {
  files.forEach(file=>{
    const pill=mk('div','tp-upload-pill'); pill.textContent=`${file.name} â€” Uploadingâ€¦`;
...
async function ingestAndAssign(file, topicId, pill, filesWrap) {
  ...
  try {
    ...
    if (!r.ok) { pill.textContent=`${file.name} â€” Upload failed.`; pill.classList.add('error'); return; }
    pill.textContent=`âœ“ ${file.name}`; pill.classList.add('done');
    await fetchTopics();
    loadDocumentTags(filesWrap, topicId);
    setTimeout(()=>{ pill.style.transition='opacity 0.5s'; pill.style.opacity='0'; setTimeout(()=>pill.remove(),500); }, 3000);
  } catch { pill.textContent=`${file.name} â€” Upload failed.`; pill.classList.add('error'); }
}

/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Classes panel (supradmin) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function loadClassesPanel() {
  ...
  const createRow=mk('div','tp-create-row');
  const nameInput=document.createElement('input');
  nameInput.type='text'; nameInput.placeholder='New class nameâ€¦'; nameInput.autocomplete='off';
...
async function renderClassCards(container) {
  container.innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:6px">Loadingâ€¦</p>';
  try {
  ...
  try {
    ...
    if (members.length) {
      members.forEach(m=>{
        ...
        tag.innerHTML=`${esc(m.username)}<button class="cls-member-del" title="Remove">Ã—</button>`;
    ...
    const sel=document.createElement('select'); sel.innerHTML=`<option value="">Add ${memberType}â€¦</option>`;
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Chats panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function renderChatsList(container) {
  container.innerHTML='<p class="admin-empty">Loadingâ€¦</p>';
...
async function viewAdminChat(classId, chatId, title, username) {
  const body=document.getElementById('panel-body');
  body.innerHTML='';
  const backBtn=mk('button','admin-back-btn'); backBtn.innerHTML='â† Back to list';
    ...
    for (let i=0; i<msgs.length; i+=2) {
      ...
      if (am) {
        ...
        if (am.feedback_rating) {
          ...
          fb.textContent=am.feedback_rating==='up'?'ðŸ‘ Helpful':'ðŸ‘Ž Not helpful';
          if (am.feedback_comment) fb.textContent+=` â€” "${am.feedback_comment}"`;
          pair.appendChild(fb);
        }
      }
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Feedback panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
async function loadFeedbackPanel() {
  ...
  rSel.innerHTML='<option value="">All ratings</option><option value="up">ðŸ‘ Positive</option><option value="down">ðŸ‘Ž Negative</option>';
  const filterBtn=document.createElement('button'); filterBtn.textContent='Filter';
...
async function renderFeedbackList(container) {
  ...
  try {
    ...
    fb.forEach(f=>{
      ...
      const ratingLbl=f.feedback_rating==='up'?'ðŸ‘ Helpful':'ðŸ‘Ž Not helpful';
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ top-k â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ New chat â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Query â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
function trunc(s,n)   { return s.length>n?s.slice(0,n)+'â€¦':s; }

async function friendlyError(res) {
  let d=''; try { d=(await res.json()).detail||''; } catch {}
  if (res.status===401) { setTimeout(showLoginModal,150); return 'ðŸ”’ Session expired â€” please sign in again.'; }
  if (res.status===403) return 'ðŸš« Access denied.';
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Feedback bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */

function appendFeedbackBar(block, chatId, msgId, existing) {
  const bar=mk('div','feedback-bar');
  const upBtn=mk('button','fb-btn up');   upBtn.innerHTML='ðŸ‘'; upBtn.title='Helpful';
  const downBtn=mk('button','fb-btn down'); downBtn.innerHTML='ðŸ‘Ž'; downBtn.title='Not helpful';
  const statusEl=mk('span','fb-status');
  const commentRow=mk('div','fb-comment-row');
  const commentIn=mk('input','fb-comment-input');
  commentIn.placeholder='Optional commentâ€¦'; commentIn.type='text';
  ...
  submitBtn.addEventListener('click', async ()=>{
    ...
    submitBtn.disabled=true; statusEl.textContent='Savingâ€¦';
    try {
      const r=await fetch(`/chats/${chatId}/messages/${msgId}/feedback`,{
        method:'POST', headers:getHeaders({'Content-Type':'application/json'}),
        body:JSON.stringify({rating:pendingRating, comment:comment||null}),
      });
      if (!r.ok) { statusEl.textContent="Couldn't save â€” try again."; submitBtn.disabled=false; return; }
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Admin panels â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
document.querySelectorAll('.reveal-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    const show=inp.type==='password'; inp.type=show?'text':'password'; btn.textContent=show?'ðŸ™ˆ':'ðŸ‘';
  });
});
...
(function() {
  ...
  if (hint) hint.innerHTML=mac?'<kbd>âŒ˜</kbd><kbd>â†µ</kbd> to send':'<kbd>Ctrl</kbd><kbd>â†µ</kbd> to send';
})();
init();
